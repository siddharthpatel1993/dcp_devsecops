pipeline{
    ...
    stages{
            ...
            steps {
                script {
                    withSonarQubeEnv(credentialsId: 'jenkins_sonarqube_token') {
                        sh "/opt/sonar-scanner/bin/sonar-scanner -Dsonar.projectKey=project_devops -Dsonar.sources=. -Dsonar.host.url=http://18.234.106.25 -Dsonar.python.coverage.reportPaths=coverage.xml -Dsonar.python.version=3 -Dsonar.projectVersion=${BUILD_NUMBER}"
                    }
                }
            }
        ...
        stage("Trivy Scan") {
            steps {
                script {
                   sh ('trivy image siddharthgopalpatel/dcp_devsecops:${IMAGE_TAG}')
                }
            }
